...
class MockReasoningProvider(BaseReasoningProvider):
    ...
    async def generate(self, query: str, context: str) -> dict:
        response = await to_thread(self.reasoner.respond, query=query, context=context)
    ...
    async def chat(self, messages: list[dict], context: str) -> dict:
        ...
        response = await to_thread(self.reasoner.respond, query=last_query, context=context)
        response["provider"] = self.name
        return response
