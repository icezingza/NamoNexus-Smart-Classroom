...
async def load_all_secrets():
    ...
    try:
        ...
        settings.jwt_secret_key = await load_secret_from_gcp("namo-jwt-secret", project_id)
        settings.admin_password = await load_secret_from_gcp("namo-admin-password", project_id)
        settings.reasoning_api_key = await load_secret_from_gcp("namo-groq-api-key", project_id)
        # Database and Redis passwords (Phase 3)
        settings.database_password = await load_secret_from_gcp("namo-database-password", project_id)
        settings.redis_password = await load_secret_from_gcp("namo-redis-password", project_id)
