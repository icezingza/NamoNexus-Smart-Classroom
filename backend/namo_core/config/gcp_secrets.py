...
async def load_all_secrets():
    ...
    try:
        ...
        settings.jwt_secret_key = (
            await load_secret_from_gcp("namo-jwt-secret", project_id)
        ).strip()
        settings.admin_password = (
            await load_secret_from_gcp("namo-admin-password", project_id)
        ).strip()
        settings.reasoning_api_key = (
            await load_secret_from_gcp("namo-groq-api-key", project_id)
        ).strip()
        # Database and Redis passwords (Phase 3)
        settings.database_password = (
            await load_secret_from_gcp("namo-database-password", project_id)
        ).strip()
        settings.redis_password = (
            await load_secret_from_gcp("namo-redis-password", project_id)
        ).strip()
        ...
        # Override SQLite with PostgreSQL using dynamic user
        db_user = os.environ.get("NAMO_DB_USER", settings.database_user)
        settings.database_url = f"postgresql://{db_user}:{settings.database_password}@{db_host}:5432/namo_classroom"
        # Set Redis URL
