---
name: github-issue-planning-en
description: Execute the "github-issue-planning-en" workflow. This skill guides through a structured workflow with defined steps and decision points.
---
...
flowchart TD
    ...
    prompt-ticket --> subagentflow-1778149813844
    subagentflow-1778149813844 --> prompt-gh-fetch
...
#### prompt-gh-fetch(get_issue) - AI Parameter Config Mode

<!-- MCP_NODE_METADATA: {"mode":"aiParameterConfig","serverId":"github","toolName":"get_issue","userIntent":"Fetch the issue details using the URL or number provided by the user in the previous step.","parameterSchema":[{"name":"issue_url_or_number","type":"string","required":true,"description":"GitHub Issue URL or issue number"}]} -->
...
```
Fetch the issue details using the URL or number provided by the user in the previous step.
```

**Parameter Schema** (for AI interpretation):

- `issue_url_or_number` (string) (required): GitHub Issue URL or issue number

**Execution Method**:

Antigravity should interpret the natural language description above and set appropriate parameter values based on the parameter schema. Use your best judgment to map the user intent to concrete parameter values that satisfy the constraints.
