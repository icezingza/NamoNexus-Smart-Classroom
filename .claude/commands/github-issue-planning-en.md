...
flowchart TD
    ...
    start-1([Start])
    end_1([End])
    mcp-1778170964058[[MCP Task: ตรวจสอบและดึงข้อมูลหมา...]]
...
#### mcp-1778170964058(MCP Auto-Selection) - AI Tool Selection Mode

<!-- MCP_NODE_METADATA: {"mode":"aiToolSelection","serverId":"github-mcp-server","userIntent":"ตรวจสอบและดึงข้อมูลหมายเลข Issue จากข้อมูลที่ได้รับในขั้นตอน prompt-ticket\r\n\r\n## กฎการทำงาน:\r\n1. หากผู้ใช้ใส่ URL เต็ม (เช่น https://github.com/owner/repo/issues/123) ให้สกัดเอาเฉพาะเลข \"123\" ออกมา\r\n2. หากผู้ใช้ใส่เป็นตัวเลขอย่างเดียว (เช่น #123 หรือ 123) ให้ตรวจสอบว่าเป็นตัวเลขที่ถูกต้อง\r\n3. ตรวจสอบว่าข้อมูลที่ส่งมาไม่ว่างเปล่า\r\n4. ส่งผลลัพธ์เป็นหมายเลข Issue เพียงอย่างเดียวเพื่อใช้ในขั้นตอน prompt-gh-fetch ต่อไป\r\n\r\n## กรณีผิดพลาด:\r\n- หากรูปแบบไม่ถูกต้อง ให้แจ้งเตือนผู้ใช้และอธิบายรูปแบบที่ต้องการ (URL หรือ #ID)\r\n"} -->
...
```
ตรวจสอบและดึงข้อมูลหมายเลข Issue จากข้อมูลที่ได้รับในขั้นตอน prompt-ticket
...
Claude Code should analyze the task description above and query the MCP server "github-mcp-server" at runtime to get the current list of tools. Then, select the most appropriate tool and determine the appropriate parameter values based on the task requirements.
